...
function Messages() {
  ...
  return (
    ...
    <div className="w-full h-[80px] flex items-center gap-[20px] px-[10px]">
        ...
        <MdOutlineKeyboardBackspace className='w-[25px] h-[25px] text-white cursor-pointer lg:hidden' onClick={()=>navigate(`/`)} />
    <h1 className="text-white text-[20px] font-semibold">Messages</h1>
    </div>


    <div className='w-full h- [80px] flex gap-[20px]
justify-start items-center overflow-x-auto p-
[20px] border-b-2
border-gray-800'>
  {userData?.following?.map((user,index)=>(
    ...
    <div className="w-full h-full overflow-auto flex flex-col gap-[20px]">
      {prevChatUsers?.map((user,index)=>(
        <div key={user._id} className="text-white cursor-pointer w-full flex items-center gap-[10px]" onClick={()=>{
...
<div className="flex flex-col">
  ...
  {onlineUsers?.includes(user._id) && <div className="text-purple-500 text-[15px]">Active Now</div>}
</div>    
