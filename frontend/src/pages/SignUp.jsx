...
const handleSignUp=async ()=>{
  ...
  return (
    ...
    items-center'>
      <div className='w-[90%] lg:max-w-[60%]
      h-[600px]
        ...
        <div className="w-full lg:w-[50%] h-full bg-white flex flex-col items-center p-[10px] gap-[20px]">
          <div className="flex gap-[10px] items-center text-[20px] font-semibold mt-[40px]  " >
            <span>Sign Up to</span>
              <span className="text-[28px] font-bold bg-gradient-to-r from-purple-600 via-pink-500 to-orange-400 bg-clip-text text-transparent">
            LOOPR
              </span>
              {/* <img src={logo} alt="LOOPR LOGO" className="w-[110px]" /> */}
          </div>
       ...
       <div  className='relative flex items-center justify-start w-[90%] h-[50px] rounded-2xl border-2 border-black '
          ...
          onClick={()=>setInputClicked({...inputClicked,password:true})} >
            ...
            <label htmlFor='password' className={`text-gray-700 absolute left-[20px] p-[5px] bg-white text-[15px] ${inputClicked.password?"top-[-15px]":""}` }>Enter Password</label>
              ...
              {!showPassword?<LuEye className=" absolute cursor-pointer right-[20px] w-[25px] h-[25px]" onClick={()=>setShowPassword(true)}  />:<LuEyeClosed  className=" absolute cursor-pointer right-[20px] w-[25px] h-[25px]" onClick={()=>setShowPassword(false)}  />}
          ...
          {err && <p className='text-red-500'>{err}</p>}
            ...
            <p className="cursor-pointer text-gray-800" onClick={()=>navigate("/signin")}>Already Have An Account ? <span className="text-black border-b-2 border-b-black pb-[3px]" >Sign In</span></p>
        ...
        <div className="md:w-[50%] h-full hidden lg:flex justify-center items-center bg-[#000000] flex-col gap-[10px] text-white text-[16px] font-semibold rounded-l-[30px] shadow-2xl shadow-black">
          <img src={logo} alt="Logo loopr" className="w-[90%]" />
        </div>
