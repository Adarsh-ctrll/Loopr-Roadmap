...
<div key={com._id} className="w-full flex flex-col gap-[5px] border-b-[1px] border-gray-800 justify-center pb-[10px] mt-[10px]">
    ...
    <div className="w-full fixed bottom-0 h-[80px] flex items-center justify-between px-[20px] py-[20px] ">
         ...
         < input type="text" className="px-[10px] border-b-2 border-b-gray-500 w-[90%] outline-none text-white placeholder:text-white h-[40px]" placeholder=' Write comment...' onChange={(e)=>setMessage(e.target.value)} value={message}/>
...
<div className="absolute top-[20px] z-[100]  right-[20px]"onClick={()=>setIsMute(prev=>!prev)}>
  {!isMute?<FaVolumeHigh className="w-[20px] h-[20px] text-white font-semibold cursor-pointer" />:<FaVolumeXmark  className="w-[20px] h-[20px] text-white font-semibold cursor-pointer"/>}
</div>

<div className="absolute bottom-0 left-0 w-full h-[5px] bg-black/40 overflow-hidden">
  <div
    className="
      h-full
      bg-gradient-to-r
  ...
  <div className="w-full h-[100px] absolute bottom-[10px] p-[10px] flex flex-col gap-[10px]" >
                ...
                <div className="w-[150px] font-semibold truncate text-white">
                  ...
                  <FollowButton targetUserId={loop.author?._id} tailwind={
   ...
   {!loop.likes.includes(userData._id) && <GoHeart className="w-[25px] cursor-pointer h-[25px]"/> }
    {loop.likes.includes(userData._id) && <GoHeartFill className="w-[25px] cursor-pointer h-[25px] text-red-600"/> }
                ...
                <div className="flex   flex-col items-center cursor-pointer " onClick={()=>{
                  ...
                  <div className="text-center">{loop.comments.length}</div>
                </div>
