...
console.log("Before:", userData.saved)
console.log("After:", result.data.saved)
  } catch (error) {
    console.log(error.response)
  }
}
...
{userData._id!=post.author._id && <FollowButton tailwind={`px-[10px] min-w-[80px] md:min-w-[100px] py-[5px] h-[30px] md:h-[40px]
bg-[black]
text-white
...
<div className="w-full h-[60px] flex justify-between items-center px-[20px] mt-[10px]">
    <div className="flex justify-cenetr items-center gap-[10px]">
...
<div className="flex justify-center items-center gap-[5px]"onClick={()=>{
...
{!userData?.saved?.includes(post?._id) && <IoBookmarkOutline className="w-[25px] cursor-pointer h-[25px]" />}
{userData?.saved?.includes(post?._id) && <IoBookmark className="w-[25px] cursor-pointer h-[25px] text-gray-600"/>}
